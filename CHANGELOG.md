...
- **行程表单**：新增「现金概览（多币种）」区块，可为单个行程配置多种现金（自定义名称/币种），分别填写后续需要、当前剩余与已使用金额
- **单位支持**：每种现金可选择以「原金额」或「K（以千为单位）」录入与展示，适配印尼盾等常用千单位货币
- **行程详情**：在行程详情页展示「现金概览」卡片，按币种汇总展示后续需要 / 已使用 / 当前剩余三项金额，并对「当前剩余」做突出显示，方便快速查看
