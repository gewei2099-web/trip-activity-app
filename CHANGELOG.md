...
## [2026-03-16] 行程多币种现金概览

- **行程表单**：新增「现金概览（多币种）」区块，可为单个行程配置多种现金（自定义名称/币种），分别填写计划需要、当前剩余与已使用金额
- **单位支持**：每种现金可选择以「个」或「千」为单位录入与展示，适配印尼盾等常用千单位货币
- **行程详情**：在行程详情页展示「现金概览」卡片，按币种汇总显示三类金额，并给出「预计够用」或「可能不够」的简单判断，帮助快速确认后续行程是否现金充足

## [2025-03-06] 活动类型与物品分类可配置

- **设置页**：新增「活动类型与物品分类」配置区，可自定义活动类型（如景点、交通）和携带物品分类
- **活动类型**：行程编辑、独立活动表单中的类型下拉使用可配置列表
- **物品分类**：携带清单的分类选择与分组展示使用可配置列表
- **导出**：配置随数据一并导出备份
...
- **分类**：证件、电子、衣物、洗漱、药品、其他
- **编辑页**：添加时选择分类，可修改已有物品分类，按分类分组展示
- **详情页**：按分类分组展示，便于收拾时逐类核对

## [2025-03-06] 活动地图「全部」视图天-序格式

- **选「全部」时**：每天第一个活动只显示天数（1、2、3…），第 2 个及以后显示「天-序」（如 3-2、3-3）
- **选某天时**：保持原有当日序号（1、2、3…）
